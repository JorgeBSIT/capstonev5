<?php
    ...
    if(isset($_POST["query"])) {
        ...
        $query = "
        SELECT * FROM equipment_inventory ORDER BY name
        ";
    }
                    ...
                    while($row = mysqli_fetch_array($result)) {
                        if($row['quantity'] > 0) {
                            ...
                            <tr>
                                ...
                                <td class="date_arrived">'.$row["date_arrived"].'</td>
                                <td class="description">'.$row["description"].'</td>
                                <td><button type="button" class="btnSelect" data-bs-toggle="modal" data-bs-target="#selectModal" style="background-color: green; color: white; border: none; border-radius: 2px; padding: 10px;">Select</button></td>
                            </tr>';
                        }
                    }
            echo $output;
    ...
    echo "<script src='distributeEquipment/edit.js'></script>";
    echo "<script src='distributeEquipment/select.js'></script>";
?>